describe("CurrentOffersCtrl", function () {
  ...
  beforeEach(inject(function($controller, $rootScope, $q) {
    ...
    state = {
      go: function() { },
      current: 'myOffers.currentOffers'
    };
    stateParams = {
      page: 2
    },
    ...
    self = $controller('CurrentOffersCtrl', {
      $scope: scope,
      $state: state,
  ...
  describe("self.retrieveJobs", function () {
    describe("when ownerService.getCurrentJobs succeeds", function () {
      it("sets the response jobs to self.jobs", inject(function ($q) {
        ...
        spyOn(ownerServiceMock, "getCurrentJobs").and.returnValue(response.promise);
        self.retrieveJobs();
    ...
    describe("when ownerService.getCurrentJobs fails", function () {
  ...
  describe("self.showTaskers", function () {
    it("goes to myTaskers.currentTaskers with the given parameters", function () {
      spyOn(state, "go");
      self.showTaskers('my-status', '10');
      expect(state.go).toHaveBeenCalledWith('myTaskers.currentTaskers', jasmine.objectContaining({
        status: 'my-status',
        job: '10'
      }));
    });
