...
.controller('PastJobsCtrl', function($stateParams, $state, taskerService, authService,
                                     alertService) {
  var self = this;
  ...
  self.retrieveJobs = function(page) {
    taskerService.getPastJobs(page)
      .then(function (data) {
        self.jobs = data.jobs;
        ...
        switch (response.status) {
          case 400:
            self.alert = alertService.buildError(response.data.error,
                                                 "Your jobs couldn't be retrieved");
            break;
          case 401:
            $state.go('landing', {});
            break;
          default:
            self.alert = alertService.buildError("Your jobs couldn't be retrieved");
            break;
        }
      });
  };
